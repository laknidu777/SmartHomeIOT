...
String webPage = R"rawliteral(
    ...
    <html>
    <head>
        <title>Fan Config</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
    </head>
    <body>
        <h2>Fan WiFi Setup</h2>
        <form action="/save">
...
void startAPMode() {
    WiFi.softAP("Fan_Config");  // Open Access Point (NO PASSWORD)
    Serial.println("AP Mode Started! Connect to 'Fan_Config' and go to 192.168.4.1");
...
void controlRelay() {
    if (WiFi.status() == WL_CONNECTED) {
              digitalWrite (LedPin, HIGH);
        HTTPClient http;
        String url = String(firebaseHost) + "/device/Fan1.json?auth=" + firebaseSecret;
        http.begin(url);
